import { err, ErrorCode, strToTokens, tokensToStr } from "@fort-major/msq-shared";
...
export class E8s {
  ...
  constructor(val: bigint) {
    if (val < 0n) {
      err(ErrorCode.INVALID_INPUT, "Unable to negate E8s");
    }
...
export class EDs {
  ...
  private assertSameDecimals(b: EDs) {
    if (this.decimals !== b.decimals) {
      err(ErrorCode.INVALID_INPUT, "Invalid EDs operation: decimal point mismatch");
    }
  }
